...
namespace ArcSoftFace
{
    public partial class FaceForm : Form
    {
        ...
        {
            Thread thread = new Thread(Docoding);
            thread.IsBackground = true;
            thread.Start();
        }

        public unsafe void Docoding()
        {
            CanRun = true;
            string url = "rtmp://192.168.160.122:1935/myapp/test2";
            ...
            var frameNumber = 0;
            AppendText(string.Format("------------------------------开始播放，时间:{0}------------------------------\n", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:ms")));
            AppendText("\n");
            while (CanRun)
            {
                try
                ...
                show(bitmap);
                //bitmap.Save(AppDomain.CurrentDomain.BaseDirectory + frameNumber + ".jpg", ImageFormat.Jpeg);
                frameNumber++;
            }
        ...
        private void show(Bitmap bitmap)
        {
            ...
            if (multiFaceInfo.faceNum < 1)
            {
                image = ImageUtil.ScaleImage(image, picImageCompare.Width, picImageCompare.Height);
                image1Feature = IntPtr.Zero;
            ...
            ASF_SingleFaceInfo singleFaceInfo = new ASF_SingleFaceInfo();
            //提取人脸特征
            image1Feature = FaceUtil.ExtractFeature(pImageEngine, image, out singleFaceInfo);
            ...
            //获取缩放比例
            float scaleRate = ImageUtil.getWidthAndHeight(image.Width, image.Height, picImageCompare.Width, picImageCompare.Height);
            //缩放图片
            image = ImageUtil.ScaleImage(image, picImageCompare.Width, picImageCompare.Height);
            //添加标记
            image = ImageUtil.MarkRectAndString(image, (int)(temp.left * scaleRate), (int)(temp.top * scaleRate), (int)(temp.right * scaleRate) - (int)(temp.left * scaleRate), (int)(temp.bottom * scaleRate) - (int)(temp.top * scaleRate), ageTemp, genderTemp, picImageCompare.Width);

            //显示标记后的图像
            picImageCompare.Image = image;
        }
        ...
        //        float offsetX = pictureBox2.Width * 1f / bitmap.Width;
        //        float offsetY = pictureBox2.Height * 1f / bitmap.Height;
        //        float x = rect.left * offsetX;
        //        float width = rect.right * offsetX - x;
        //        float y = rect.top * offsetY;
        //        float height = rect.bottom * offsetY - y;
